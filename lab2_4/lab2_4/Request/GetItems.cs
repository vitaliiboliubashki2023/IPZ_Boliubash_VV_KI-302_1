...
public class MedicineService
{
    ...
    public static List<Item> FilterMedicines(List<Item> items, string searchName, string selectedCategory)
    {
        var filtered = items.Where(medicine =>
            (string.IsNullOrEmpty(searchName) || medicine.Name.ToLower().Contains(searchName.ToLower())) &&
            (selectedCategory == "Усі" || medicine.Type == selectedCategory)).ToList();

        return new List<Item>(filtered);
    }
}